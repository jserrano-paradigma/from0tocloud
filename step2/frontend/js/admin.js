...
async function addElement(type){
  ...
  if (baseurl !== 'http://localhost:3000'){
    ...
    if (type === 'user'){ 
      schema.orders = []; 
      schema = await add(services.origin.prourl + services.branches.baseurl + services.branches.users, JSON.stringify(schema)); 
      addUserCard(schema);
    }
    if (type === 'product'){  
      schema.category = null;
      schema = await add(services.origin.prourl + services.branches.baseurl + services.branches.items, JSON.stringify(schema));
      addProductCard(schema);
    }
    if (type === 'category'){ 
      schema = await add(services.origin.prourl + services.branches.baseurl + services.branches.category, JSON.stringify(schema));
...
async function deleteUser(id){
  users = await del(services.origin.prourl + services.branches.baseurl + services.branches.users+'/'+id);
  $('#'+id).hide('slow', function(){  $('#'+id).remove(); });
}

async function deleteItem(id){
  users = await del(services.origin.prourl + services.branches.baseurl + services.branches.items+'/'+id);
  $('#'+id).hide('slow', function(){  $('#'+id).remove(); });
}

async function deleteCategory(id){
  users = await del(services.origin.prourl + services.branches.baseurl + services.branches.category+'/'+id);
  $('#'+id).hide('slow', function(){  $('#'+id).remove(); });
}

//Get data functions services call
async function getUsers(){
  users = await get(services.origin.prourl + services.branches.baseurl + services.branches.users);
  isLogged();
  $("#user-spinner").toggle();
...
async function getProducts(){
  products = await get(services.origin.prourl + services.branches.baseurl + services.branches.items);
...
async function getCategories(){
  categories = await get(services.origin.prourl + services.branches.baseurl + services.branches.category);
